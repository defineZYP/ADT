...
class DisenDataset(Dataset):
    def __init__(self, args, user_seq, test_neg_items=None, data_type='train', eval_set=-1):
        self.args = args
        self.user_seq = user_seq
        self.test_neg_items = test_neg_items
        self.data_type = data_type
        ...
        if eval_set == -1:
            self.data_length = len(self.user_seq)
        else:
            self.data_length = min(eval_set, len(self.user_seq))
    ...
    def __getitem__(self, index):
        ...
        target_pos = [0] * pad_len + target_pos
        target_neg = [0] * pad_len + target_neg
        pad_len = self.max_len - len(dec_ids)
        dec_ids = [0] * pad_len + dec_ids
