...
class LayerNorm(nn.Module):
    def __init__(self, hidden_units, eps=1e-12):
        """Construct a layernorm module in the TF style (epsilon inside the square root).
        """
        super(LayerNorm, self).__init__()
        self.weight = nn.Parameter(torch.ones(hidden_units))
        self.bias = nn.Parameter(torch.zeros(hidden_units))
...
class Embeddings(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.item_embeddings = nn.Embedding(args.item_size, args.hidden_units, padding_idx=0)
        self.position_embeddings = nn.Embedding(args.maxlen, args.hidden_units)
    ...
    def __init__(self, args):
        ...
        if args.hidden_units % args.num_heads != 0:
            raise ValueError(
                ...
                "heads (%d)" % (args.hidden_units, args.num_heads))
        self.num_heads = args.num_heads
        self.attention_head_size = int(args.hidden_units / args.num_heads)
        self.all_head_size = self.num_heads * self.attention_head_size

        self.query = nn.Linear(args.hidden_units, self.all_head_size)
        self.key = nn.Linear(args.hidden_units, self.all_head_size)
        self.value = nn.Linear(args.hidden_units, self.all_head_size)

        self.attn_dropout = nn.Dropout(args.attention_dropout)

        self.dense = nn.Linear(args.hidden_units, args.hidden_units)
        self.LayerNorm = LayerNorm(args.hidden_units, eps=1e-12)
...
class DistAttention(nn.Module):
    def __init__(self, args):
        ...
        self.mean_query = nn.Linear(args.hidden_units, self.all_head_size)
        self.cov_query = nn.Linear(args.hidden_units, self.all_head_size)
        self.mean_key = nn.Linear(args.hidden_units, self.all_head_size)
        self.cov_key = nn.Linear(args.hidden_units, self.all_head_size)
        self.mean_value = nn.Linear(args.hidden_units, self.all_head_size)
        self.cov_value = nn.Linear(args.hidden_units, self.all_head_size)

        self.activation = nn.ELU()

        self.attn_dropout = nn.Dropout(args.attention_dropout)
        self.mean_dense = nn.Linear(args.hidden_units, args.hidden_units)
        self.cov_dense = nn.Linear(args.hidden_units, args.hidden_units)
        self.out_dropout = nn.Dropout(args.dropout)
    ...
    def forward(self, input_mean_tensor, input_cov_tensor, attention_mask, rec=False):
        ...
        rec_mean_context_layer = mean_context_layer.permute(0, 2, 1, 3).contiguous() # batch_size, maxlen, num_heads, int(hidden_units/num_heads)
        rec_cov_context_layer = cov_context_layer.permute(0, 2, 1, 3).contiguous()

        new_context_layer_shape = rec_mean_context_layer.size()[:-2] + (self.all_head_size,)

        mean_context_layer = rec_mean_context_layer.view(*new_context_layer_shape)
        cov_context_layer = rec_cov_context_layer.view(*new_context_layer_shape)
        ...
        if not rec:
            ...
            return mean_hidden_states, cov_hidden_states, attention_probs, rec_mean_context_layer, rec_cov_context_layer

class DistEDAttention(nn.Module):
    def __init__(self, args):
        super(DistEDAttention, self).__init__()
        if args.hidden_units % args.num_heads != 0:
            raise ValueError(
                "The hidden size (%d) is not a multiple of the number of attention "
                "heads (%d)" % (args.hidden_units, args.num_heads))
    ...
    def forward(self, input_mean_tensor, input_cov_tensor, enc_mean_tensor, enc_cov_tensor, attention_mask):
        mixed_mean_query_layer = self.mean_query(input_mean_tensor)
        mixed_mean_key_layer = self.mean_key(enc_mean_tensor)
        mixed_mean_value_layer = self.mean_value(enc_mean_tensor)
        ...
        mixed_cov_query_layer = self.activation(self.cov_query(input_cov_tensor)) + 1
        mixed_cov_key_layer = self.activation(self.cov_key(enc_cov_tensor)) + 1
        mixed_cov_value_layer = self.activation(self.cov_value(enc_cov_tensor)) + 1
...
class Intermediate(nn.Module):
    def __init__(self, args):
        ...
        self.dense_2 = nn.Linear(args.hidden_units * 4, args.hidden_units)
        self.LayerNorm = LayerNorm(args.hidden_units, eps=1e-12)
        self.dropout = nn.Dropout(args.dropout)
...
class DistLayer(nn.Module):
    def __init__(self, args):
        ...
        self.attention = DistAttention(args)
        self.mean_intermediate = DistIntermediate(args)
        self.cov_intermediate = DistIntermediate(args)
        self.mean_independence_layer = nn.Linear(int(args.hidden_units / args.num_heads), args.num_heads)
        self.cov_independence_layer = nn.Linear(int(args.hidden_units / args.num_heads), args.num_heads)
        self.activation_func = nn.ELU()

    def forward(self, mean_hidden_states, cov_hidden_states, attention_mask):
        mean_attention_output, cov_attention_output, attention_scores, rec_mean, rec_cov = self.attention(mean_hidden_states, cov_hidden_states, attention_mask, True)
        ...
        rec_mean = self.mean_independence_layer(rec_mean)
        rec_cov = self.cov_independence_layer(rec_cov)
        return mean_intermediate_output, cov_intermediate_output, attention_scores, rec_mean, rec_cov

class DistDecLayer(nn.Module):
    def __init__(self, args):
        super(DistDecLayer, self).__init__()
        self.dec_attention = DistAttention(args)
        self.enc_attention = DistEDAttention(args)
        self.mean_intermediate = DistIntermediate(args)
        self.cov_intermediate = DistIntermediate(args)
        self.activation_func = nn.ELU()

    def forward(self, mean_hidden_states, cov_hidden_states, enc_hidden_states, enc_cov_states, attention_mask, trg_attention_mask):
        mean_attention_output, cov_attention_output, _ = self.dec_attention(mean_hidden_states, cov_hidden_states, attention_mask)
        mean_attention_output, cov_attention_output, _ = self.enc_attention(mean_hidden_states, cov_hidden_states, enc_hidden_states, enc_cov_states, trg_attention_mask)
        mean_intermediate_output = self.mean_intermediate(mean_attention_output)
        cov_intermediate_output = self.activation_func(self.cov_intermediate(cov_attention_output)) + 1
        return mean_intermediate_output, cov_intermediate_output
...
class DistSAEncoder(nn.Module):               
    ...
    def forward(self, mean_hidden_states, cov_hidden_states, attention_mask, output_all_encoded_layers=True):
        ...
        for layer_module in self.layer:
            ...
            mean_hidden_states, cov_hidden_states, att_scores, rec_mean, rec_cov = layer_module(mean_hidden_states, cov_hidden_states, attention_mask)
...
class DistSADecoder(nn.Module):
    def __init__(self, args):
        super(DistSADecoder, self).__init__()
    ...
    def forward(self, mean_hidden_states, cov_hidden_states, enc_hidden_states, enc_cov_states, attention_mask, trg_attention_mask):
        decoder_layers = []
        for layer_module in self.layer:
            mean_hidden_states, cov_hidden_states = layer_module(mean_hidden_states, cov_hidden_states, enc_hidden_states, enc_cov_states, attention_mask, trg_attention_mask)
